...
import argparse
import ipaddress
from lib.device import get_device_info, get_app_info, launch_app
...
def main():
    ...
    group.add_argument('-a', '--app', type=int, help='Launch an app with a particular ID')
    group.add_argument('-l', '--list_apps', action='store_true', help='List all apps and their IDs')

    args = parser.parse_args()
    setup_logging(args.debug)

    if not any([args.command, args.string, args.youtube, args.app, args.list_apps]):
        args.info = True
    ...
    for roku_ip in targets:
        ...
        if args.list_apps:
            logger.info(f'Querying app info at {roku_ip}...')
            app_info = get_app_info(roku_ip)
        
            print()
            title_str = f'APPS'
            print(title_str)
            print('=' * len(title_str))
            max_length = max(len(app["text"]) for app in app_info)
            for app in app_info:
                print(f'* {app['text']:<{max_length}} (id: {app['id']})')
            print()
        ...
        if args.app:
            logger.info(f'Launching app {args.app}')
            for app in get_app_info(roku_ip):
                if app['id'] == args.app:
                    launch_app(roku_ip, args.app)
                    break
            else:
                logger.error(f'App with ID {args.app} not found.')
